...
const i_url = 'https://a7275984.eu-gb.apigw.appdomain.cloud/bnkapi-cust/customer';
...
function main(a_params) {
    ...
    if ( !isDefined(a_params.cust_id) ){
            var err = new Error(i_logid + 'Не передан параметр a_params.cust_id') ;
            throw err;
    } 


    let l_reqparams = {  headers: i_headers, params: { id: a_params.cust_id}};


    //var l_url = i_url + '?'+'valcode='+a_params.valcode+'&'+'date='+l_exchdt+'&'+'json'
    
    //l_step = i_logid + 'Вызываю сервис: [' + l_url + ']';
    console.log(l_step + '-2');    
